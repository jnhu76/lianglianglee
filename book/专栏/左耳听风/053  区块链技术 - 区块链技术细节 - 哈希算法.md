...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="053  区块链技术 - 区块链技术细节 - 哈希算法" class="title">053  区块链技术 - 区块链技术细节 - 哈希算法</h1>
...
<p>每个数据块的 &ldquo; 地址 &rdquo; 的编码使用了计算机上的一个算法，计算机圈内人士把这个算法叫 Secure Hash。有人音译为 &ldquo; 安全哈希 &ldquo;，也有人意译为 &rdquo; 安全散列 &ldquo;。在计算机应用中，hash 算法主要有几个功能。</p>
...
<p>于是，很多公司在互联网上发布信息或软件的时候，都会带上一个 Checksum（校验码）。你只要把整个文件的数据传入到那个特定的 hash 算法中，就会得到一串很长的字符串。如果和官方发布的 Checksum 字符串不一样，那么就说明信息或文件内容被人更改或是信息残缺了。因此，也被应用在 &rdquo; 数字签名 &ldquo; 中。</p>
...
<ul>
<li>对 &ldquo;chen hao&rdquo; 这个信息计算 MD5 值得到 9824df83b2d35172ef5eb63a47a878eb（一个 16 进制数）。</li>
<li>如果对 “chen hao&rdquo; 做一个字符的修改，将字母 &ldquo;o&rdquo; 改成数字 &ldquo;0”，即 “chen ha0”，计算出来的 MD5 值就成了 d521ce0616359df7e16b20486b78d2a8。可以看到，这和之前的 MD5 值完全不一样了。</li>
</ul>

<p>于是，我们就可以利用 hash 算法的这个特性来对数据做 &rdquo; 数字签名 &ldquo;。也就是说，我将 &rdquo; 数据 &ldquo; 和其 &rdquo; 签名 &ldquo;（hash 计算值）一起发布，这样可以让收到方来验证数据有没有被修改。</p>

<p>我们再来看上面那个区块链的图。</p>

<p><img src="assets/1c75fed53ee023c353a31616d2b29e62.png" alt="img" /></p>

<p>对于第一块数据，我们把其 &rdquo; 数据集 &ldquo; 和 &rdquo; 前数据块的 hash 值 00000a6cba&rdquo; 一起做 hash 值，得到本区块的地址 000007cabfa。然后，下一个区块会把自己的数据和 000007cabfa 一起做 hash，得到 000008acbed 这个哈希值……如此往复下去。</p>

<p>根据 &ldquo; 被 hash 的数据中有一个 bit 被修改了，整个 hash 就完全不一样了 &rdquo; 这个特性，我们知道：</p>
...
<li>Timestamp：区块生成的时间。这个时间不能早于前面 11 个区块的中位时间，不能晚于 &ldquo; 网络协调时间 &ldquo;——你所连接的所有结点时间的中位数。4 个字节。</li>
...
<p>当然，事情并没有这么简单。比特币对这个 hash 值是有要求的，其要求是那个 Bits 字段控制的，然后你可以调整 Nonce 这个 32 位整型的值来找到符合条件的 hash 值。我们把这个事情叫做 &rdquo; 挖矿 &ldquo;（在下一篇中，我们会详细讲一下这个事）。</p>
...
<p>上面的示意图中有四笔交易，A 和 B 的 hash 成了 Hash-AB， C 和 D 的 hash 成了 Hash-CD，然后再做 Hash-AB + Hash-CD 的 hash，得到了 Hash-ABCD，这就是 Merkle Root。整个过程就像一个 &rdquo; 二叉树 &ldquo; 一样。</p>
...
<li>还有一个是用来做交易收据的。主要是用来记录一个智能合约中最终发生的交易信息。在 StackExchange 上的问题 &ldquo;<a href="https://ethereum.stackexchange.com/questions/5888/relationship-between-transaction-trie-and-receipts-trie" target="_blank">Relationship between Transaction Trie and Receipts Trie</a>&rdquo; 中有相应的说明，你可以前往一看。</li>
</ul>
...
<p>于是，一笔交易可能会包含大量的 Input 和 Output。因为比特币没有 &ldquo; 余额 &rdquo; 的概念，所以需要通过多个 input 来凑，然后 output 这边还需要给自己 &ldquo; 找零 &ldquo;，给矿工 &rdquo; 小费 &ldquo;。</p>
...
<script data-cfasync="false" src="/static/email-decode.min.js"></script><script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'8f1469119a3b9508',t:'MTczNDA3NjcyMC4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/static/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
