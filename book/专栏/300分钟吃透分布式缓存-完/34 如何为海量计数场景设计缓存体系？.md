...
<head>
    ...
    <head>
        ...
        <meta name='description' content=34&#32;如何为海量计数场景设计缓存体系？>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="34 如何为海量计数场景设计缓存体系？" class="title">34 如何为海量计数场景设计缓存体系？</h1>
                            <div><p>在上一课时我们讲解了如何为秒杀系统进行缓存设计，在本课时我们将具体讲解如何为海量计数场景设计缓存服务。</p>

<h2 id="计数常规方案">计数常规方案</h2>

<p><img src="assets/CgpOIF4EfGeAMy7uAACOifnCjYI046.png" alt="img" /></p>
...
<p><img src="assets/Cgq2xl4EfKKATQvhAAB_y6G_3Y0242.png" alt="img" /></p>
...
<p>因为，一方面 Redis 作为通用型存储来存储计数，内存存储效率低。以存储一个 key 为 long 型 id、value 为 4 字节的计数为例，Redis 至少需要 65 个字节左右，不同版本略有差异。但这个计数理论只需要占用 12 个字节即可。内存有效负荷只有 <sup>12</sup>&frasl;<sub>65</sub>=18.5%。如果再考虑一个 long 型 id 需要存 4 个不同类型的 4 字节计数，内存有效负荷只有 (8+16)/(65*4)= 9.2%。</p>
...
<p><img src="assets/Cgq2xl4EfNaAVYSOAACwXN0Zx5E804.png" alt="img" /></p>
...
<p><img src="assets/CgpOIF4EfUeALwp6AACe1xvi1Sc311.png" alt="img" /></p>
