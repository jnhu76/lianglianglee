...
<p>虽然，从使用方式上看，Ant 匹配器和 MVC 匹配器并没有什么区别，但在日常开发过程中，我想推荐你<strong>使用 MVC 匹配器</strong>而不是 Ant 匹配器，原因就在于 Ant 匹配器在匹配路径上有一些风险，主要<strong>体现在对于&rdquo;/&ldquo;的处理上</strong>。为了更好地说明，我举一个简单的例子。</p>
