...
<head>
    ...
    <head>
        ...
        <meta name='description' content=加餐03&#32;理解ftrace（1）：怎么应用ftrace查看长延时内核函数？>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="加餐03 理解ftrace（1）：怎么应用ftrace查看长延时内核函数？" class="title">加餐03 理解ftrace（1）：怎么应用ftrace查看长延时内核函数？</h1>
                            <div><p>你好，我是程远。</p>
...
<p>输出里&rdquo;do_mount &lt;- ksys_mount&rdquo;表示do_mount()函数是被ksys_mount()这个函数调用到的，&rdquo;2159455.499195&rdquo;表示函数执行时的时间戳，而&rdquo;[005]&ldquo;是内核函数do_mount()被执行时所在的CPU编号，还有&rdquo;mount-20889&rdquo;，它是do_mount()被执行时当前进程的pid和进程名。</p>
...
<p>Linux内核在编译的时候，缺省会使用三个gcc的参数&rdquo;-pg -mfentry -mrecord-mcount&rdquo;。</p>

<p>其中，&rdquo;-pg -mfentry&rdquo;这两个参数的作用是，给编译出来的每个函数开头都插入一条指令&rdquo;callq &lt;<strong>fentry</strong>&gt;&ldquo;。</p>

<p>你如果编译过内核，那么你可以用&rdquo;objdump -D vmlinux&rdquo;来查看一下内核函数的汇编，比如do_mount()函数的开头几条汇编就是这样的：</p>
...
<p>而&rdquo;-mrecord-mcount&rdquo;参数在最后的内核二进制文件vmlinux中附加了一个mcount_loc的段，这个段里记录了所有&rdquo;callq &lt;<strong>fentry</strong>&gt;&ldquo;指令的地址。这样我们很容易就能找到每个函数的这个入口点。</p>

<p>为了方便你理解，我画了一张示意图，我们编译出来的vmlinux就像图里展示的这样：</p>

<p><img src="assets/9f62b0951b764fa61b7e5fe9b2d05449.jpeg" alt="img" /></p>

<p>不过你需要注意的是，<strong>尽管通过编译的方式，我们可以给每个函数都加上一个额外的hook点，但是这个额外&rdquo;fentry&rdquo;函数调用的开销是很大的。</strong></p>

<p>即使&rdquo;<strong>fentry</strong>&ldquo;函数中只是一个retq指令，也会使内核性能下降13%，这对于Linux内核来说显然是不可以被接受的。那我们应该怎么办呢？</p>

<p>ftrace在内核启动的时候做了一件事，就是把内核每个函数里的第一条指令&rdquo;callq &lt;<strong>fentry</strong>&gt;&ldquo;（5个字节），替换成了&rdquo;nop&rdquo;指令（0F 1F 44 00 00），也就是一条空指令，表示什么都不做。</p>

<p>虽然是空指令，不过在内核的代码段里，这相当于给每个函数预留了5个字节。这样在需要的时候，内核可以再把这5个字节替换成callq指令，call的函数就可以指定成我们需要的函数了。</p>

<p>同时，内核的mcount_loc段里，虽然已经记录了每个函数&rdquo;callq &lt;<strong>fentry</strong>&gt;&ldquo;的地址，不过对于ftrace来说，除了地址之外，它还需要一些额外的信息。</p>
...
<p>当我们需要用function tracer来trace某一个函数的时候，比如&rdquo;echo do_mount &gt; set_ftrace_filter&rdquo;命令执行之后，do_mount()函数的第一条指令就会被替换成调用ftrace_caller的指令。</p>
...
<p>我们讲ftrace实现机制时，说过内核中的“inline函数”不能被ftrace到，你知道这是为什么吗？那么内核中的&rdquo;static函数&rdquo;可以被ftrace追踪到吗？</p>
