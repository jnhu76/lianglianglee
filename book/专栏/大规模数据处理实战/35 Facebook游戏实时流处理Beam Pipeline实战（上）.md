...
<head>
    ...
    <head>
        ...
        <meta name='description' content=35&#32;Facebook游戏实时流处理Beam&#32;Pipeline实战（上）>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="35 Facebook游戏实时流处理Beam Pipeline实战（上）" class="title">35 Facebook游戏实时流处理Beam Pipeline实战（上）</h1>
                            <div><p>你好，我是蔡元楠。</p>
...
<p><img src="assets/7d5b23478bcf4bf8b2d790ccadff041f.jpg" alt="" /></p>

<p>当然了，除了消除游戏本身带来的乐趣以外，可以在Facebook里和自己的好友进行积分排名比拼也是另外一个能吸引用户的地方。</p>

<p><img src="assets/5899c12f93034fc086e7c736589bf35f.jpg" alt="" /></p>
...
<li>自定义一个Composite Transform，其中包括三个步骤：利用Top Transform将每一个用户的最高分率选出来，得到PCollection<KV<String, List >&gt;；将PCollection<KV<String, List \>&gt;转换成为PCollection<KV<String, UserScoreInfo>&gt;；再次利用Top Transform将PCollection<KV<String, UserScoreInfo>&gt;中前100名高分用户筛选出来。</li>
...
<p>在上面这个例子中，输入的参数是每一行字符串PCollection ，输出结果是每一个单词对应出现的次数PCollection<KV<String, Long>。在实现expand这个抽象方法的时候，里面总共嵌套了两个不同的Transform，分别是一个ParDo用来提取每一行的单词，还有一个Count Transform统计单词出现的次数。</p>
...
<p>例如，我们想要得到2019年7月15日至2019年7月21日这周的排行榜，那在运行数据流水线的时候，参数就可以按照“–startBoundary=2019-07-15-00-00 &ndash;etartBoundary=2019-07-21-00-00”传入了。</p>
