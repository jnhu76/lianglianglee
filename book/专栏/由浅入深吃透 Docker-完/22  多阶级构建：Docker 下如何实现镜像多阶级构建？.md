...
<head>
    ...
    <head>
        ...
        <meta name='description' content=22&#32;&#32;多阶级构建：Docker&#32;下如何实现镜像多阶级构建？>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="22  多阶级构建：Docker 下如何实现镜像多阶级构建？" class="title">22  多阶级构建：Docker 下如何实现镜像多阶级构建？</h1>
...
<p>使用第二个 FROM 命令表示镜像构建的第二阶段，使用 COPY 指令拷贝编译后的文件到 alpine 镜像中，&ndash;from=0 表示从第一阶段构建结果中拷贝文件到当前构建阶段。</p>
...
<p>我们在第一个构建阶段，使用 AS 指令将这个阶段命名为 builder。然后在第二个构建阶段使用 &ndash;from=builder 指令，即可从第一个构建阶段中拷贝文件，使得 Dockerfile 更加清晰可读。</p>
