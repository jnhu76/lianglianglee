...
<head>
    ...
    <head>
        ...
        <meta name='description' content=14&#32;深入浅出解读&#32;Kafka&#32;的可靠性机制>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="14 深入浅出解读 Kafka 的可靠性机制" class="title">14 深入浅出解读 Kafka 的可靠性机制</h1>
...
<p>If a follower hasn&rsquo;t sent any fetch requests or hasn&rsquo;t consumed up to the leaders log end offset for at least this time, the leader will remove the follower from ISR.</p>
</blockquote>
...
props.put(&ldquo;bootstrap.servers&rdquo;, &ldquo;100.120.130.170:9092,100.120.130.171:9092, 100.120.130.172:9092&rdquo;);
props.put(&ldquo;acks&rdquo;, &ldquo;all&rdquo;); //保证高可靠性，设置成&rdquo;all&rdquo;或者&rdquo;-1&rdquo;
props.put(&ldquo;retries&rdquo;, 3);  //重试次数阈值，这里设置为3
props.put(&ldquo;key.serializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringSerializer&rdquo;); //这里是key的序列化类
props.put(&ldquo;value.serializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringSerializer&rdquo;);//这里是value的序列化类
...
<p>Future<RecordMetadata> future = producer.send(newProducerRecord<String, String>(&ldquo;test.testTopic&rdquo;, &ldquo;key&rdquo;,&ldquo;value&rdquo;));</p>
...
props.put(&ldquo;bootstrap.servers&rdquo;, &ldquo;100.120.160.170:9092,100.120.160.171:9092, 100.120.160.172:9092&rdquo;);
props.put(&ldquo;acks&rdquo;, &ldquo;all&rdquo;); // 高可靠性要求则应该设置成&rdquo;all&rdquo;
props.put(&ldquo;retries&rdquo;, Integer.MAX_VALUE); //启用重试机制
props.put(&ldquo;max.in.flight.requests.per.connection&rdquo;,1); //避免消息重排序
props.put(&ldquo;key.serializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringSerializer&rdquo;); //这里是key的序列化类
props.put(&ldquo;value.serializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringSerializer&rdquo;);//这里是value的序列化类</p>

<p>Producer<String, String> producer = new KafkaProducer<String,String>(props);</p>

<p>for (int i = 0; i &lt; 1000000; i++) {</p>

<p>Future<RecordMetadata> future = producer.send(new ProducerRecord<String, String>(&ldquo;test.testTopic&rdquo;,&ldquo;key&rdquo;,&ldquo;value&rdquo;));</p>
...
If true the consumer&rsquo;s offset will be periodically committed in the background.</p>
...
<p>Properties props = new Properties();
props.put(&ldquo;bootstrap.servers&rdquo;, &ldquo;100.120.160.170:9092,100.120.160.171:9092, 100.120.160.172:9092&rdquo;);//配置broker集群
props.put(&ldquo;group.id&rdquo;, &ldquo;testsub1&rdquo;);// 为consumer配置group
props.put(&ldquo;enable.auto.commit&rdquo;, &ldquo;false&rdquo;); // 设置手动提交
props.put(&ldquo;key.deserializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringDeserializer&rdquo;);
props.put(&ldquo;value.deserializer&rdquo;, &ldquo;org.apache.kafka.common.serialization.StringDeserializer&rdquo;);</p>

<p>KafkaConsumer<String, String> consumer = new KafkaConsumer<String,String>(props);</p>

<p>consumer.subscribe(Arrays.asList(&ldquo;test.testTopic&rdquo;,&ldquo;topic.test&rdquo;));</p>
