...
<head>
    ...
    <head>
        ...
        <meta name='description' content=09&#32;&#32;如何回答&#32;MySQL&#32;的索引原理与优化问题？>
...
<body>
    ...
    overflow-y: hidden;">
                        <div class="book-post">
                            ...
                            <h1 id="title" data-id="09  如何回答 MySQL 的索引原理与优化问题？" class="title">09  如何回答 MySQL 的索引原理与优化问题？</h1>
...
<p><strong>我们结合普通索引的 B+Tree 结构看一下索引失效的原因：</strong> 当 MySQL 优化器根据 name like ‘%路由器’ 这个条件，到索引 index_name 的 B+Tree 结构上进行查询评估时，发现当前节点的左右子节点上的值都有可能符合 &lsquo;%路由器&rsquo; 这个条件，于是优化器判定当前索引需要扫描整个索引，并且还要回表查询，不如直接全表扫描。</p>

<p>当然，还有其他类似的索引失效的情况：</p>

<ul>
<li>索引列上做了计算、函数、类型转换操作，这些情况下索引失效是因为查询过程需要扫描整个索引并回表，代价高于直接全表扫描；</li>
<li>like 匹配使用了前缀匹配符 &lsquo;%abc&rsquo;；</li>
<li>字符串不加引号导致类型转换；</li>
</ul>
